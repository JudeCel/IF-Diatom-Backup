...
test('getReportRowObjects_VotingTests.js', function (t) {
    var sessionId = 0;
    var topicId = 0;
    var user1Id = 0;
    var user2Id = 0;
    ...
    t.test("Gets all events of the Topic", function (t) {
        ...
        var resCb = function (result) {
            ...
            t.equal(result[2].name, userName2, "result[2].name is correct");
    ...
    t.test("Gets events of the Topic excluding Facilitator events", function (t) {
        ...
        var resCb = function (result) {
            ...
            t.equal(result[1].isFirst, false, "result[1].isFirst is correct");
            t.equal(result[1].isLast, true, "result[1].isLast is correct");
            t.equal(result[1].user_id, user1Id, "result[1].user_id is correct");
            t.equal(result[1].name, userName1, "result[1].name is correct");
...
function run(params, resCb, nextCb) {
    var getReportDataCb = function (data) {
        resCb(voteReportHandler.getReportRowObjects(data, nextCb));
    }
    voteReportHandler.getReportData(params, getReportDataCb, nextCb);
}