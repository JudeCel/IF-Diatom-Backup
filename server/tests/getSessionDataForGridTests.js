...
test('Gets participants', function (t) {
    ...
    t.test("Sets up a session, topic and user", function (t) {
        ...
        t.test("Get session staff, unfiltered", function (t) {
            var params = {};
            var resCb = function (data) {
                t.ok(data, "Info was returned");
                if (data!=undefined)
                {
                    t.ok((data.length>=1), "Found The Staff");
                }
                t.end();
            };
    ...
    t.test("Get session staff with companyId", function (t) {
        var params = {
            companyId:companyId
        };
        var resCb = function (data) {
            ...
            if (data!=undefined)
            {
                t.equals(data.length, 1, "Found The Staff");
            }
            t.end();
        };
        var nextCb = function (data) {
...
function run(params, resCb, nextCb) {
	...
	var cmd = require('../handlers/getSessionDataForGrid.js');
